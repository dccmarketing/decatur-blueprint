...
function decaturblueprint_scripts() {
	...
	wp_enqueue_script( 'decatur-blueprint-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20120206', true );

	wp_enqueue_script( 'decatur-blueprint-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );
	...
	if ( is_home() || is_front_page() ) {
		...
		wp_enqueue_script( 'decatur-blueprint-bg-video', get_template_directory_uri() . '/js/html5ext.js' );
...
function decaturblue_menu_icons( $item_output, $item, $depth, $args ) {
	...
	$output .= get_page_icon( $item->title );
	$output .= '</a>';
